...
- **Alice 企业画像师**两档：快速补全（一次检索）/ 完整画像流水线（与「企业画像工具」同一条链路，写入企业库与融资 / 动态 / 管理团队子实体）。厂长排产时按用户措辞选 `profile_mode`；单聊说「跑完整画像」即走完整流水线。
- **Leo 赛事雷达员**（新工序「赛事雷达」，在企业画像之后）：按奖励导向（设备 / 钱 / 实习 / offer / 算力）找企业办的比赛，写入赛事库；总任务里每家企业一条检索，单聊里可按主题或企业派活。
- **Nova 质检**简报新增：相关赛事数（报名中 / 给设备 / 给实习或 offer）、企业画像平均完整度与跑过完整流水线的家数。
